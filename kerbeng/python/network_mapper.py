  ...
  for key,value in interfacesDict.iteritems():
    ...
    for interface in value:
      ...
      for name_pattern in LINK_SPEEDS:
        ...
        if pattern.match(interface['Local_Interface']):
          bIsPhysical = True;
          onlyPhysicalInterfacesDict[key].append({"Local_Interface":interface['Local_Interface'],
      ...
      print(str(bIsPhysical) + " - Local_Interface:" + interface['Local_Interface'] + " is physical.")
  ...
  for key,value in interfacesDict.iteritems():
    ...
    for interface in value:
      if interface['oper_status'] == 'UP':     
        onlyUpInterfacesDict[key].append({"Local_Interface":interface['Local_Interface'],
                                          "oper_status":interface['oper_status'],
                                          "admin_status":interface['admin_status'],
                                          "actual_bandwith":interface['actual_bandwith'],
                                          "description":interface['description']})   
        print("Local_Interface:" + interface['Local_Interface'] + " is OPRATIONAL.")
  ...
  for devicename,neiInterfaceDict in neighborships.iteritems():
    ...
    for interface in interfacesDict[devicename]:
      ...
      for Neighbor_Interface in neiInterfaceDict:
        print("Local_Interface: " + interface['Local_Interface'] 
           + " Neighbor_Interface['Local_Interface']:" + Neighbor_Interface['Local_Interface'])
        if interface['Local_Interface'] == Neighbor_Interface['Local_Interface']:
      ...
      if bHasNoNeighbor:
        ...
        print("Neighborless Interface on device: " + devicename + " int:" + interface['Local_Interface'])  
...
def getInterfaces(device):
  ...
  for index in range(len(intName)):
    ...
    deviceActiveInterfacesDict.append({"Local_Interface":intName[index].text,
